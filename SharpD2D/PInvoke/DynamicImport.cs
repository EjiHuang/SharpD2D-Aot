...
namespace SharpD2D.PInvoke
{
    internal static class DynamicImport
    {
        ...
        public static IntPtr ImportMethod(IntPtr moduleHandle, string methodName)
        {
            ...
            if (procAddress == IntPtr.Zero)
                throw new DynamicImportException(
                    $"DynamicImport failed to find method \"{methodName}\" in module \"0x{moduleHandle.ToString("X")}\"!");
            return procAddress;
        }
    ...
    internal class DynamicImportException : Win32Exception
    {
        ...
        public DynamicImportException(string message) : base(message + Environment.NewLine + "ErrorCode: " +
